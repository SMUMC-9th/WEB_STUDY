...
const useDeleteComments = () => {
    ...
    return useMutation({
        ...
        onMutate: async({lpid, commentId, order}) => {
            ...
            const previousComments = queryClient.getQueriesData<InfiniteData<ResponseCommentListDto>>(queryKey);
        ...
        onError: (error, variables, context) => {
            if(context?.previousComments) {
                queryClient.setQueriesData(context.queryKey, context.previousComments);
            }
