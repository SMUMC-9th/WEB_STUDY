...
export const MovieDetailError = ({ 
    ...
    const getErrorInfo = () => {
        if (errorStates.movieDetail) {
            return {
                emoji: "😵",
                title: "영화 정보를 불러올 수 없어요",
                description: "요청하신 영화를 찾을 수 없거나 일시적인 오류가 발생했습니다.",
                showAllButtons: true
            };
        } else if (errorStates.credits) {
            return {
                emoji: "👥❌", 
                title: "출연진 정보 로딩 실패",
                description: "출연진 정보를 가져올 수 없지만, 영화 정보는 정상적으로 표시됩니다.",
