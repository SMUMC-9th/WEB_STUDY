...
// 화면에서 실제로 쓰는 최소 타입
type Author = { id: number; name: string; avatarUrl?: string };
type Comment = {
  id: number;
  content: string;
  authorId: number;
...
function CommentSection({ lpId }: { lpId: string }) {
  ...
  const { mutate: createComment, isPending: creating } = useMutation({
    ...
    onSuccess: () => {
      setCommentInput("");
      // 정렬·대상 LP에 맞게 목록 무효화 → 최신 데이터로 갱신
  ...
  // 무한 스크롤 목록
  // 서버 스펙: { data: { data: Comment[]; hasNext: boolean; nextCursor: number } }
  const {
    data,
    fetchNextPage,
    hasNextPage,
  ...
  const flatComments: Comment[] = useMemo(
    () => data?.pages.flatMap((p) => p.data.data as Comment[]) ?? [],
    [data]
  );
